...
def opencv_gb_recognize():
    ...
    if ret and time.time() >= gb_recognize_time:
        ...
        if point_blue and point_green:
            ...
            delta_y = point_green[1] - point_blue[1]
    ...
    for obstacle_positions in obstacle_positions_list:
        for i in range(2):
            ...
            if i == 0:
                for j in range(len(obstacle_positions)):
                    ...
                    if 0 < current_x < map_width and 0 < current_y < map_height:
                        if j > 0:
                            ...
                            cv2.line(my_map, (previous_x, previous_y), (current_x, current_y), (255, 255, 255), 50)
                ...
                cv2.line(my_map, (first_x, first_y), (last_x, last_y), (255, 255, 255), 50)
...
def opencv_path_map(path):
    ...
    cv2.circle(path_map, (start_x, start_y), 5, (255, 0, 0), -1)
    cv2.circle(path_map, (goal_x, goal_y), 5, (0, 0, 255), -1)
    ...
    if len(path) == 0:
        ...
        for i in range(len(path)):
            cv2.circle(path_map, (path[i, 1] * resize_ratio, path[i, 0] * resize_ratio), 1, (0, 0, 255), -1)
        return path_map
...
cv2.namedWindow("My map path")

my_map_path = np.zeros((map_height, map_width, 3), dtype=np.uint8)
...
my_map_path = cv2.add(my_map, path_map)
cv2.imshow("My map path", my_map_path)
